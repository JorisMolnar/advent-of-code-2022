...
export class Program {
  ...
  private pourGrains (paths: Path[]): number {
    ...
    const solid = new Set(pathsToPoints(paths).map(pointToHash))
    ...
    while (true) {
      ...
      while (true) {
        // First check if we are lower than the lowest constuction. Then we are entering The Abyss™.
        if (sand.y > ymax) return grains
